...
net_names = ["isnet_is", "isnet", "isnet_gt", "u2net", "u2netl", "modnet"]
...
class CustomSegmentation(pl.LightningModule,
                        PyTorchModelHubMixin,
                        library_name="segmentation",
                        repo_url="https://github.com/tuanha1305/segmentation",
...
def get_gt_encoder(train_dataloader, val_dataloader, opt):
    ...
    gt_encoder = CustomSegmentation("isnet_gt")
...
def main(opt):
    ...
    if opt.pretrained_ckpt == "":
        custom_seg = CustomSegmentation(opt.net, opt.img_size)
    else:
        custom_seg = CustomSegmentation.try_load(opt.net, opt.pretrained_ckpt, "cpu", opt.img_size)
    if not opt.pretrained_ckpt and not opt.resume_ckpt and opt.net == "isnet_is":
        custom_seg.gt_encoder.load_state_dict(get_gt_encoder(train_dataloader, val_dataloader, opt).state_dict())
    custom_seg.lr = opt.lr
    ...
    trainer.fit(custom_seg, train_dataloader, val_dataloader, ckpt_path=opt.resume_ckpt or None)
...
if __name__ == "__main__":
    ...
    # dataset args
    parser.add_argument('--data-dir', type=str, default='../../dataset/custom-seg',
